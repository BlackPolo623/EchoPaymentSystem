...
// atm_redirect.php
// 接收歐買尬金流 ClientRedirectURL 回傳的取號資料
// ============================================
...
ini_set('error_log', $dataDir . '/atm_redirect.log');

// 記錄接收到的資料
$logId = uniqid('ATM_REDIRECT_');
$logData = "[" . date('Y-m-d H:i:s') . "][$logId] 接收到 ClientRedirectURL 回調\n";
$logData .= "POST數據: " . print_r($_POST, true) . "\n";
file_put_contents($dataDir . '/atm_redirect.log', $logData, FILE_APPEND);
...
if (isset($receivedData['CheckMacValue'])) {
    ...
    $logData = "[" . date('Y-m-d H:i:s') . "][$logId] CheckMac驗證: " . ($isValidCheckMac ? 'SUCCESS' : 'FAILED') . "\n";
    file_put_contents($dataDir . '/atm_redirect.log', $logData, FILE_APPEND);
}
